...
function App() {
  ...
  const cargarSecciones = () => {
    ...
    const url = `http://localhost:3001/api/contenido?cacheBust=${new Date().getTime()}`;
    axios.get(url)
      .then(res => {
  ...
  const handleSave = () => {
    ...
    if (contenidoSeleccionado.tipo === 'seccion') {
        ruta = `http://localhost:3001/api/secciones/${contenidoSeleccionado.id}`;
    } else if (contenidoSeleccionado.tipo === 'subseccion') {
        ruta = `http://localhost:3001/api/subsecciones/${contenidoSeleccionado.id}`;
    } else {
        return;
    }
  ...
  const handleCrearElemento = ({ titulo, parentId, tipo, orden }) => {
    const url = 'http://localhost:3001/api/elementos';
  ...
  const handleDeleteItem = (node) => {
    setConfirmState({
      ...
      onConfirm: () => {
        let url = '';
        if (node.tipo === 'capitulo') url = `http://localhost:3001/api/capitulos/${node.id}`;
        else if (node.tipo === 'seccion') url = `http://localhost:3001/api/secciones/${node.id}`;
        else if (node.tipo === 'subseccion') url = `http://localhost:3001/api/subsecciones/${node.id}`;
        else return;
  ...
  const guardarTitulo = (node, nuevoTitulo, onFinish) => {
    let url = '';
    if (node.tipo === 'capitulo') url = `http://localhost:3001/api/capitulos/${node.id}/titulo`;
    else if (node.tipo === 'seccion') url = `http://localhost:3001/api/secciones/${node.id}/titulo`;
    else if (node.tipo === 'subseccion') url = `http://localhost:3001/api/subsecciones/${node.id}/titulo`;
    else return;
